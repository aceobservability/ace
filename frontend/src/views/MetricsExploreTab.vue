...
<template>
  <div class="flex flex-col gap-6 flex-1" @keydown="handleKeydown">
    <div class="flex flex-col gap-4 rounded-lg p-4" :style="{ backgroundColor: 'var(--color-surface-container-low)' }">
      <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-4 items-end max-md:grid-cols-1">
        <div class="flex flex-col gap-2.5">
          <label class="text-xs font-semibold uppercase tracking-wide" :style="{ color: 'var(--color-outline)' }">Data Source</label>
          <div ref="datasourceMenuRef" class="relative">
            ...
            >
              <template v-if="activeDatasource">
                <img :src="getTypeLogo(activeDatasource.type)" :alt="`${dataSourceTypeLabels[activeDatasource.type]} logo`" class="h-7 w-7 shrink-0 object-contain" />
                <div class="flex flex-col min-w-0 gap-px">
                  <span class="text-[0.68rem] uppercase tracking-wide" :style="{ color: 'var(--color-outline)' }">Active Source</span>
                  <strong class="text-sm font-semibold truncate" :style="{ color: 'var(--color-on-surface)' }">{{ activeDatasource.name }}</strong>
                  <span class="font-mono text-xs uppercase tracking-[0.07em]" :style="{ color: 'var(--color-outline)' }">{{ dataSourceTypeLabels[activeDatasource.type] }}</span>
                </div>
                <span
              ...
              <span v-else class="text-sm" :style="{ color: 'var(--color-outline)' }">No metrics datasource configured</span>
              <component :is="showDatasourceMenu ? ChevronUp : ChevronDown" :size="16" class="ml-1 shrink-0" :style="{ color: 'var(--color-outline)' }" />
            </button>

            <div v-if="showDatasourceMenu && hasMetricsDatasources" class="absolute left-0 right-0 top-full mt-1.5 z-[110] max-h-[280px] overflow-y-auto rounded-lg shadow-lg" :style="{ backgroundColor: 'var(--color-surface-bright)', border: '1px solid var(--color-outline-variant)' }">
              <button
                v-for="ds in metricsDatasources"
                :key="ds.id"
                type="button"
                class="flex w-full items-center gap-2.5 border-none bg-transparent px-3 py-2.5 text-left cursor-pointer transition"
                :style="{
                  color: 'var(--color-on-surface)',
                  backgroundColor: ds.id === selectedDatasourceId ? 'color-mix(in srgb, var(--color-primary) 15%, transparent)' : 'transparent',
                }"
                ...
                @mouseleave="($event.currentTarget as HTMLElement).style.backgroundColor = ds.id === selectedDatasourceId ? 'color-mix(in srgb, var(--color-primary) 15%, transparent)' : 'transparent'"
              >
                <img :src="getTypeLogo(ds.type)" :alt="`${dataSourceTypeLabels[ds.type]} logo`" class="h-[18px] w-[18px] shrink-0 object-contain" />
                <div class="flex min-w-0 flex-col gap-px">
                  <strong class="text-sm font-semibold" :style="{ color: 'var(--color-on-surface)' }">{{ ds.name }}</strong>
                  <span class="text-xs" :style="{ color: 'var(--color-outline)' }">{{ dataSourceTypeLabels[ds.type] }}</span>
                </div>
                <Check v-if="ds.id === selectedDatasourceId" :size="14" class="ml-auto" :style="{ color: 'var(--color-primary)' }" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2.5">
          <label class="text-xs font-semibold uppercase tracking-wide" :style="{ color: 'var(--color-outline)' }">Query Range</label>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-if="queryHistory.length > 0" class="relative">
          <button
            ...
            class="flex items-center gap-1 text-sm cursor-pointer bg-transparent border-none transition"
          ...
          <div v-if="showHistory" class="absolute left-0 top-full mt-1 z-10 w-80 max-h-[300px] overflow-y-auto rounded-lg shadow-lg max-md:w-full" :style="{ backgroundColor: 'var(--color-surface-bright)', border: '1px solid var(--color-outline-variant)' }">
            <div class="flex items-center justify-between px-4 py-3 text-xs font-semibold uppercase tracking-wide" :style="{ color: 'var(--color-outline)', borderBottom: '1px solid var(--color-outline-variant)' }">
              <span>Recent Queries</span>
              <button class="flex items-center justify-center h-6 w-6 rounded bg-transparent border-none cursor-pointer transition" :style="{ color: 'var(--color-outline)' }" @click="clearHistory" title="Clear history">
                <X :size="14" />
              </button>
            </div>
            <button
              ...
              :key="index"
              class="block w-full border-none bg-transparent px-4 py-2.5 text-left cursor-pointer transition"
              :style="{ borderBottom: '1px solid var(--color-outline-variant)' }"
              @click="selectHistoryQuery(q)"
              @mouseenter="($event.currentTarget as HTMLElement).style.backgroundColor = 'var(--color-surface-container-high)'"
              @mouseleave="($event.currentTarget as HTMLElement).style.backgroundColor = 'transparent'"
            >
              <code class="block font-mono text-xs truncate" :style="{ color: 'var(--color-on-surface-variant)' }">{{ q }}</code>
            </button>
          </div>
        </div>
      </div>
      ...
      <div
        v-if="error"
        class="flex items-center gap-2 rounded-sm p-4 text-sm"
        :style="{
    ...
    <!-- Results section -->
    <div class="flex flex-1 flex-col rounded-lg overflow-hidden min-h-[400px]" :style="{ backgroundColor: 'var(--color-surface-container-low)' }">
      <div v-if="loading" class="flex flex-col items-center justify-center gap-4 py-12 flex-1" :style="{ color: 'var(--color-outline)' }">
        <div class="animate-spin h-8 w-8 rounded-full" :style="{ border: '3px solid var(--color-outline-variant)', borderTopColor: 'var(--color-primary)' }"></div>
      ...
      <div v-else-if="result?.status === 'success' && chartSeries.length === 0" class="flex flex-col items-center justify-center py-12 text-center text-sm flex-1" :style="{ color: 'var(--color-outline)' }">
        <p class="m-0">No data returned for the selected time range.</p>
      </div>

      <div v-else-if="!hasMetricsDatasources" class="flex flex-col items-center justify-center py-12 text-center text-sm flex-1" :style="{ color: 'var(--color-outline)' }">
        <p class="m-0">No metrics datasource configured.</p>
        <p class="m-0 text-xs">Add a Prometheus, VictoriaMetrics, CloudWatch, or Elasticsearch datasource in Data Sources.</p>
      </div>

      <div v-else class="flex flex-col items-center justify-center py-12 text-center text-sm flex-1" :style="{ color: 'var(--color-outline)' }">
